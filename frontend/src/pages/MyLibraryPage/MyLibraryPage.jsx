...
export default function MyLibraryPage() {
  ...
  return <main className='myLibrary'>
    ...
    {loading ? <p role='status'>Loading library…</p> : selected.length === 0 && <p>{books.length ? 'No books match these filters.' : 'Your library is empty. Accept a recommendation to add your first book.'}</p>}
    {rows.map((row, index) => <div key={index}><div className='displayed-books'>{row.map(book => <button type='button' className='book-select' key={book._id} onClick={() => setCurrent(book)}><img className='book-picture' src={book.thumbnail || '/book-placeholder.svg'} alt={book.title} onError={event => { event.currentTarget.onerror = null; event.currentTarget.src = '/book-placeholder.svg'; }} /></button>)}</div><img src={woodshelf} className='shelf' alt='' /></div>)}
